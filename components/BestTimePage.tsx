...
export default function BestTimePage() {
  const { bestTime: raw } = useSiteContent();
  const page = { ...DEFAULT_CONTENT.bestTime, ...raw };
    ...
    <main className="min-w-0 overflow-x-clip pb-[env(safe-area-inset-bottom)]">
      <section className="explore-hub visa-hub relative isolate overflow-hidden">
        ...
        <div className="explore-hub-shell contact-shell relative">
          ...
          <div className="explore-hub-glass contact-glass">
            <h2 className="about-h2 font-[family-name:var(--font-cormorant)]">{page.calendarTitle}</h2>
            <p className="contact-lead">{page.calendarIntro}</p>
          ...
          {page.seasons.map((season, index) => (
            <div key={season.id} className="explore-hub-glass contact-glass about-copy-panel">
              {index === 0 ? (
                <h2 className="about-h2 font-[family-name:var(--font-cormorant)]">{page.seasonsTitle}</h2>
              ) : null}
              <p className="explore-hub-eyebrow">{season.eyebrow}</p>
              <h3 className="visa-h3 font-[family-name:var(--font-cormorant)]">{season.title}</h3>
              <p className="visa-note">{season.meta}</p>
              {paragraphs(season.body).map((para) => (
                <p key={para.slice(0, 48)}>{para}</p>
              ))}
              <ul className="seasons-chips">
          ...
          <div className="explore-hub-glass contact-glass about-copy-panel">
            <h2 className="about-h2 font-[family-name:var(--font-cormorant)]">{page.regionsTitle}</h2>
            <p className="contact-lead">{page.regionsIntro}</p>
          ...
          <div className="explore-hub-glass contact-glass">
            <h2 className="about-h2 font-[family-name:var(--font-cormorant)]">{page.altitudeTitle}</h2>
            <p className="contact-lead">{page.altitudeIntro}</p>
            <div className="visa-table-wrap">
              <table className="visa-table">
                <thead>
                  <tr>
