...
export default function OrbitAltitudeEditor({ content, setContent }: Props) {
  const page = content.altitude ?? DEFAULT_CONTENT.altitude;

  function patch(partial: Partial<SiteContent["altitude"]>) {
    setContent({ ...content, altitude: { ...page, ...partial } });
  }

  return (
    <div className="space-y-8">
      <p className="rounded-lg border border-gold/25 bg-black/25 px-3 py-2 text-sm text-white/75">
        Public page <strong className="text-gold">/altitude-tips</strong> — glass travel-guide copy, no content photos.
      </p>
